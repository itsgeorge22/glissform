...
- Keeps the current effect in its original Space during a Space-switch animation instead of showing the same frozen image on both sides. It then clears the effect; the next animation remains disarmed until you reopen above **Begin at** and close through it in the newly active Space.
...
- Space changes intentionally stop the current animation. Opening above **Begin at** is required before a new closing gesture can animate in the newly active Space; physical confirmation of full-screen transitions remains pending.
