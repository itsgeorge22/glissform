...
- Takes one screenshot per closing attempt. A fast downward movement may request it just before your chosen lid angle; the image remains hidden until that angle is crossed. No continuous screen recording or idle capture.
...
- Timestamps lid readings when acquired and uses recent reading cadence for critically damped smoothing. It prefers fractional angles when the sensor provides them and otherwise uses whole degrees. Tiny fractional jitter is ignored for animation; slow sub-degree changes receive a gentler response, while sustained reversals brake smoothly from the current speed. This trades some tracking delay for steadier motion. Polling requests 60 reads per second during gestures and 30 while idle; the sensor's actual update rate remains a hardware limit. Rendering follows the display's presentation clock and pauses when settled.
- On a fast close, requests its single screenshot after confirmed downward movement up to three degrees before the selected start angle, but keeps it hidden until the angle is crossed. Reversing or pausing before the threshold discards that image. It replaces the live desktop with a prepared, full-opacity flat screenshot before a 100 ms geometric entrance. Manual reopening stays on the same lid-driven smoothing through the final degree. It does not start a separate return animation at the starting angle; the snapshot fades out over 50 ms only after the tracked image is flat. Pause restoration uses balanced acceleration without overshoot over 340–600 ms depending on the fold; cached wake opening uses a 50 ms entrance fade. The refined tracking and returns are unreleased and still require physical lid validation.
...
- Diffuses the entire image into a frosted-glass appearance as the lid closes, including the bottom. Fine text and icons soften while larger shapes retain enough definition to show the perspective skew; a smooth vertical gradient keeps the blur lighter at the hinge and heavier at the top. The projected image fades into the surrounding black through a continuous soft edge. Reopening clears it continuously.
- Builds a Gaussian blur pyramid once from each screenshot, samples finer levels with a wider kernel for smoother bright details, filters and shades in linear light, and uses a subtle fixed dither to reduce visible steps in broad frost and shadow gradients. The flat screenshot remains sharp and unchanged by the material.
...
- The lid sensor exposes an undocumented protocol. Fractional readings are available on the development Mac, with whole-degree fallback on unsupported hardware; their accuracy and update rate are not guaranteed. Smoothing cannot recover motion the sensor never reported.
...
The [LidAngleSensor project](https://github.com/samhenrigold/LidAngleSensor) and [Clamshell](https://github.com/danielradosa/clamshell) were consulted for the HID reports. Glissform's sensor implementation and visual effect were written independently; no competitor animation code or assets are bundled.
