...
Motion checks exercise thresholds, automatic two-second resting-angle learning and fallback, repeated closure, slow whole-degree readings, reversal, settling, zero-effect entrance, interrupted handoffs, low-lid startup, and reset behavior. They also check that the separate Settings angle display suppresses alternating one-degree reports, accepts a sustained one-degree change after 250 ms, follows slow one-degree steps and larger movements, and resets after a sensor gap. A synthetic lifecycle check verifies that changing the manual fallback does not cancel a capture anchored to a learned angle. Quantized sensor traces run on 60/120 Hz frame clocks, including uneven report intervals and mixed one/two-degree reports. Acquisition-time checks separate sample history from render queries, cover batched delivery and future presentation queries, and verify bounded velocity-aware returns. Further checks reject alternating one-degree direction chatter, accept sustained fine reversals, preserve velocity through real reversal and programmatic stops, and verify lower pause-return peak speed at shallow and deep folds. Manual completion is interrupted by sleep, display changes, and quit through the real coordinator. Renderer timeline checks compare the final manual degree and a reclose frame-for-frame against uninterrupted lid smoothing, hold one degree below the threshold without triggering a return, and allow slow settling beyond 800 ms without fading early. Both tracking error and frame-to-frame speed ripple are bounded; checking delay alone can reward visibly uneven movement. Full-angle checks cover starting angles from 20° to 130°. Requested polling rates are not measurements of real sensor update frequency. Fractional report 7 returned 101.61–101.65° while report 1 stayed at 102° in a short stationary probe on the development Mac; whether the app feels smoother during physical motion remains unverified.
...
A local comparison against checkpoint `c2fa862` used the existing synthetic 30 Hz whole-degree traces rendered at 60/120 Hz. At 2–7 degrees/second, relative speed variation fell by about 20–30%, with roughly 24–31 ms more average tracking delay; medium-speed results were similar, and faster traces also became steadier. These are algorithm measurements, not sensor-to-screen latency or physical acceptance. The implementation keeps sensor-target prediction disabled.
...
- Compare slow and fast threshold crossings on a static desktop and moving content. On fast closes, check whether the early screenshot reduces time to first visible motion; stop or reverse just above the start angle and confirm no overlay appears. Repeat after waiting above the angle, and verify the next real gesture still captures once. Confirm the prepared closing frame starts flat and full opacity before geometry changes. Record capture delay and screenshot freshness; the earlier request cannot guarantee that capture finishes before a very fast threshold crossing.
- Move very slowly through consecutive degrees, then stop abruptly or reverse by one degree and hold. Compare the new fractional-report build with the previous whole-degree build for visible jitter and final-opening continuity. Compare steady speed, lag, chatter, and deliberate fine adjustments, especially whether the gentler sub-degree response improves motion without making the lid feel delayed; also accelerate and decelerate through larger movements.
- Compare ordinary reopening, reclose during return, and pause restoration at shallow and deep folds. During manual reopening, hold one degree below the starting angle, then finish slowly or reverse. It must remain lid-controlled without a separate return at the threshold. The image must reach flat before its 50 ms fade, with restored mouse access at completion. Check for threshold braking, overshoot, geometric jumps and temporal ghosting. A changed post-sleep desktop must be evaluated separately from a static one.
- Compare fine detail, diagonal edges, coloured shapes, bright player controls and smooth dark gradients through the full rotation range. Inspect the fixed dither at normal viewing distance and in flat areas. Check for coarse blur cells or a grid on bright controls, and verify linear-light frost does not introduce objectionable brightness changes, border halos or a visible level transition.
...
On hardware, enable Resume desktop after a pause, set a 95° trigger, open above 95°, then close to 80° and hold. Confirm the effect accelerates gently toward flat, settles without a bounce, and restores desktop input; subsequent movement below 95° must not retrigger it. Try shallower and deeper folds to judge the 340–600 ms return. Reopen above 95° and close to verify a new gesture. The app uses a fixed two-second delay. Repeat with slow movement, direction changes, sleep, and toggle changes. Check that the toggle survives relaunch and that older saved custom delays have no effect.
